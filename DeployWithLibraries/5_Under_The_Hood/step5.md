...
![Solidity Compiler](https://github.com/ethereum/remix-workshops/raw/master/DeployWithLibraries/2_Generate_Metadata/remix_icon_solidity.png "Solidity Compiler")
  ...
  - コンパイルされたコントラクトのリストから`sampleContract`コントラクトを選択します。
  - `ByteCode`をクリックすると、以下がクリップボードにコピーされます。
...
これは基本的にコンパイラが返すものです：
  - `linkReferences`は、コントラクトで使用されるライブラリを記述します。
  - `object`はコンパイルされたコントラクト（バイトコード）です。 これは、ブロックチェーンにデプロイおよび保存されるものです。 この例では、バイトコード内の値 `__ $ d42d70ba92b626965f4c69b39148e37a33 $ __`は、ライブラリアドレスの単なるプレースホルダーです。 あなたの場合、プレースホルダーは`__$`と`$__`の間にあります。

Remix IDEのメタデータJSONは、プレースホルダーを指定されたアドレスに置き換えるようにRemixに指示します。
